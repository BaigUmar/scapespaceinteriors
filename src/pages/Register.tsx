...
const Register: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-brand-cream px-6 py-20">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-md w-full bg-white p-8 border border-brand-stone/10 shadow-sm"
      >
        <h2 className="text-3xl font-serif text-brand-charcoal mb-8 text-center uppercase tracking-widest">Register</h2>
        
        {error && <div className="bg-red-50 text-red-600 p-4 mb-6 text-sm border-l-2 border-red-600">{error}</div>}
        
        <form onSubmit={handleRegister} className="space-y-6">
          <div>
            <label className="block text-xs font-semibold tracking-widest text-brand-stone uppercase mb-2">Full Name</label>
            <input 
              required
              type="text"
              value={name}
